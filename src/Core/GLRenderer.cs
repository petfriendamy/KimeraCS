...
namespace KimeraCS.Rendering
{
    ...
    public static class GLRenderer
    {
        ...
        /// <summary>
        /// Set up matrices from legacy camera state.
        /// </summary>
        public static void SetupMatricesFromLegacy(float panX, float panY, float distance,
                                                    float alpha, float beta, float gamma,
                                                    int viewportWidth, int viewportHeight)
        {
            float aspect = (float)viewportWidth / viewportHeight;
            ProjectionMatrix = CreatePerspectiveMatrix(45.0f, aspect, 0.1f, 100000.0f);

            var rotX = Matrix4.CreateRotationX(MathHelper.DegreesToRadians(alpha));
            var rotY = Matrix4.CreateRotationY(MathHelper.DegreesToRadians(beta));
            var rotZ = Matrix4.CreateRotationZ(MathHelper.DegreesToRadians(gamma));
            var translation = Matrix4.CreateTranslation(panX, panY, -distance);

            ViewMatrix = translation * rotZ * rotY * rotX;
        ...
        public static void DrawLinesModern(LineMesh lineMesh)
        {
            ...
            ctx.LineShader.SetMatrix4("view", ViewMatrix);
